	...
	public void executeJunitTestCase(ProductGeneration tools) {
		...
		for (Combination combination : tools.getCombsForTest()) {
			for (FeatureCombination f : combination.getListFeatures()) {
				...
				} else if (f.getName().equals("REMOVER")) {
					Configuration.REMOVER = (f.getState().equals("0") ? false : true);
				} else if (f.getName().equals("LOGGIN")) {
			...
			if (Configuration.validProduct()) {
				...
				org.junit.runner.Result result = junit.run(LastTaskLabelTest.class, TaskEditorPanelTest_.class,
						TaskEditorPanelTest.class, TaskHistoryPanelTest.class, TaskManagerWindowTest.class,
						TaskSelectorPanelTest.class, TaskTest.class);
				Configuration.productPrint();
				System.out.println("\n\n ----------------------- \n\n");
			} else {
