...
//		Configuration.CUT_WHATEVER = true;
		if (Configuration.GUI && Configuration.TOTAL_WALKER && Configuration.CUT_WHATEVER) {
			...
			JTextComponentFixture totalValue = window.textBox(getJTextAreaMatcher("262712.0"));
		 ...
		 //   assertEquals(totalValue.text().toString(),"262712.0");
...
//		Configuration.CUT_WHATEVER = true;
		if (Configuration.GUI && Configuration.TOTAL_WALKER && Configuration.CUT_WHATEVER) {
			...
			// getting the value of "total" in the text component
			JTextComponentFixture totalValue = window.textBox(getJTextAreaMatcher("234567.0"));

			// change name of manager
			window.textBox(getJTextAreaMatcher("Ray")).requireEditable().requireEnabled().selectAll().deleteText()
					.enterText("Fubar").click();
			// change address
			window.textBox(getJTextAreaMatcher("Redmond")).requireEditable().requireEnabled().selectAll().deleteText()
					.enterText("Moon");
			...
			assertEquals(totalValue.text().toString(),"234567.0");
...
//		Configuration.LOGGING = true;
		if (Configuration.LOGGING && Configuration.GUI && Configuration.TOTAL_WALKER && Configuration.CUT_WHATEVER) {
			...
			try {
				...
				List<String> logs = Arrays.asList("Salary of Ray adjusted to 117283.5",
						"Salary of Klaus adjusted to 11728.0", "Salary of Karl adjusted to 1172.5",
...
//		Configuration.CUT_NO_DEPARTMENT = true;
		...
		if (Configuration.GUI && Configuration.TOTAL_WALKER && Configuration.CUT_NO_DEPARTMENT) {
			...
			// getting the value of "total" in the text component
			JTextComponentFixture totalValue = window.textBox(getJTextAreaMatcher("399747.0"));
			...
			// asserting that the value of "total" was changed
			// after clicking on the button "cut"
			//assertEquals(totalValue.text().toString(),"399747.0");
...
//		Configuration.CUT_NO_MANAGER = true;
		...
		if (Configuration.GUI && Configuration.TOTAL_WALKER && Configuration.CUT_NO_MANAGER) {
			...
			// asserting that the value of "total" was changed
			// after clicking on the button "cut"
			assertEquals(totalValue.text().toString(),"201046.0");
			// DEP cut
			...
			// asserting that the value of "total" was changed
			// after clicking on the button "cut"
			assertEquals(totalValue.text().toString(),"58641.75");
...
//		Configuration.PRECEDENCE = true;
		...
		if (Configuration.GUI && Configuration.CUT_WHATEVER && Configuration.TOTAL_REDUCER
			...
			// change name of manager
			window.textBox(getJTextAreaMatcher("234567.0")).requireEditable().requireEnabled().selectAll().deleteText()
					.enterText("1").click();
	...
	private GenericTypeMatcher<JTextComponent> getJTextAreaMatcher(final String text) {
